...
function useAllBeneficiaries() {
  const projectQuery = useQuery({
    queryKey: ['projects'],
    queryFn: () => createProjectService(getSDKApiUrl()).list(),
  })

  const projects = projectQuery.data ?? []

  const beneficiaryQueries = useQueries({
    queries: projects.map((p) => ({
      queryKey: ['beneficiaries', p.id],
      queryFn: () => createBeneficiaryService(getSDKApiUrl()).list(p.id),
      enabled: projects.length > 0,
    })),
  })

  const isLoading =
    projectQuery.isLoading || beneficiaryQueries.some((q) => q.isLoading)

  const raw: Beneficiary[] = beneficiaryQueries.flatMap((q) => q.data ?? [])

  // Deduplicate by id (a beneficiary may appear in multiple project queries)
  const seen = new Set<string>()
  const beneficiaries = raw.filter((b) => {
    if (seen.has(b.id)) return false
    seen.add(b.id)
    return true
  })

  return { beneficiaries, isLoading, projectCount: projects.length }
}
...
export function ReportsPage() {
  const { beneficiaries, isLoading, projectCount } = useAllBeneficiaries()
  ...
  // Status
  const verified = beneficiaries.filter((b) => b.status === 'Verified').length
  ...
  const other = beneficiaries.filter((b) => b.gender === 'Other').length
  ...
  const avgAge =
    ages.length > 0
      ...
      : 0
  const age0_17 = ages.filter((a) => a <= 17).length
  const age18_35 = ages.filter((a) => a >= 18 && a <= 35).length
  const age36_50 = ages.filter((a) => a >= 36 && a <= 50).length
  const age51plus = ages.filter((a) => a >= 51).length
  const maxAgeGroup = Math.max(age0_17, age18_35, age36_50, age51plus, 1)

  // Household size
  const householdSizes = beneficiaries
    .filter((b) => b.householdSize !== undefined)
    .map((b) => b.householdSize!)
  const avgHousehold =
    householdSizes.length > 0
      ? (
          householdSizes.reduce((s, h) => s + h, 0) / householdSizes.length
        ).toFixed(1)
      : '—'
    ...
    <div className="flex flex-col h-full overflow-y-auto bg-white">
      ...
      <div className="px-8 pt-8 pb-6 border-b border-gray-100">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Beneficiary demographics across {projectCount} project
          {projectCount !== 1 ? 's' : ''}
        </p>
      </div>
