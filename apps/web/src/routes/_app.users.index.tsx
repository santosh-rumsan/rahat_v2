...
function Users() {
  ...
  const selected = users.find((u) => u.id === effectiveSelectedId) ?? filtered[0] ?? null
          ...
          {filtered.map((u) => (
            <button
              ...
              className={cn(
                ...
                u.id === effectiveSelectedId ? 'bg-white shadow-sm' : 'hover:bg-white/50'
              )}
            >
          ...
          <div className="px-8 pt-7 pb-8">
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() =>
                    navigate({ to: '/users/$userId/edit', params: { userId: selected.id } })
                  }
